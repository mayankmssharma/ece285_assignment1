...
class CrossEntropyLoss(BaseLayer):
    ...
    def forward(self, input_x: np.ndarray, target_y: np.ndarray):
        ...
        y_onehot = np.zeros((target_y.size, target_y.max()+1))
        y_onehot[np.arange(target_y.size), target_y] = 1
        loss = np.sum(-np.log(np.sum(y_onehot * input_x, axis =1)))
    ...
    def backward(self):
        ...
        N, _ = x_temp.shape
        y_onehot = np.zeros((y_temp.size, y_temp.max()+1))
        y_onehot[np.arange(y_temp.size), y_temp] = 1
